...
Instrument InstrumentType entity attributes validation- Shares sedols
    ${row}=     Read number of rows(sql)  Shares
    ${attribute}=     Read number of rows  InstrumentType
    ${data}=    Read Sedol from excel sheet  Shares  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Sedol}  Read Sedol from excel sheet  Shares  ${i}  1
        FOR   ${j}    IN RANGE   2     ${attribute}+1
      ${DB Query0}  Read Excel Data of Cell  InstrumentType  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} '${Sedol}'
      Log to console  ${DB Query}
...
Instrument InstrumentType entity attributes validation- BondandGilts sedols
    ${row}=     Read number of rows(sql)  BondandGilts
    ${attribute}=     Read number of rows  InstrumentType
    ${data}=    Read Sedol from excel sheet  BondandGilts  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Sedol}  Read Sedol from excel sheet  BondandGilts  ${i}  1
        FOR   ${j}    IN RANGE   2     ${attribute}+1
      ${DB Query0}  Read Excel Data of Cell  InstrumentType  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} '${Sedol}'
      Log to console  ${DB Query}
