...
Instrument Metric entity attributes validation- Investment sedols
    ...
    ${attribute}=     Read number of rows  Metric
    ${data}=    Read Sedol from excel sheet  Investment  1  1
      ...
      ${DB Query0}  Read Excel Data of Cell  Metric  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} '${Sedol}'
      Log to console  ${DB Query}
      ${Source_Table_Script_Output}=              Query      ${DB Query};
      ${GQL_QueryP1}=    Read Excel Data of Cell     Metric  ${j}  3
      ${GQL_QueryP2}=    Read Excel Data of Cell     Metric  ${j}  4
      ${GQL_Query}=  Set Variable   ${GQL_QueryP1}${Sedol}"}}) ${GQL_QueryP2}
      Log to console  ${GQL_Query}
      ${JSON_PATH}=    Read Excel Data of Cell     Metric  ${j}  5
      #${Source_Table_Script_Output}=              Query      ${DB Query};
