...
Instrument FundDetails entity attributes validation- Fund sedols
    ${row}=     Read number of rows(sql)  Funds
    ${attribute}=     Read number of rows  FundDetailsFunds
    ${data}=    Read Sedol from excel sheet  Funds  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Sedol}  Read Sedol from excel sheet  Funds  ${i}  1
        FOR   ${j}    IN RANGE   2     ${attribute}+1
      ${DB Query0}  Read Excel Data of Cell  FundDetailsFunds  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} '${Sedol}'
      Log to console  ${DB Query}
      ${Source_Table_Script_Output}=              Query      ${DB Query};
      ${GQL_QueryP1}=    Read Excel Data of Cell     FundDetailsFunds  ${j}  3
      ${GQL_QueryP2}=    Read Excel Data of Cell     FundDetailsFunds  ${j}  4
      ${GQL_Query}=  Set Variable   ${GQL_QueryP1}${Sedol}"]}}) ${GQL_QueryP2}
      Log to console  ${GQL_Query}
      ${JSON_PATH}=    Read Excel Data of Cell     FundDetailsFunds  ${j}  5
      #${Source_Table_Script_Output}=              Query      ${DB Query};
      ...
      log  ${Target_Graphql_Output2}
           # IF   ${source_DB_output1}==${Target_Graphql_Output2}== []       CONTINUE
      #CONTINUE FOR LOOP IF    "${source_DB_output1}"=="${Target_Graphql_Output2}"== "[]"   AS "SKIP"
      ...
      RUN KEYWORD IF  "${passed}" == "FAIL"   run keyword And Continue on failure   FAIL
       # CONTINUE FOR LOOP IF  "${source_DB_output1}" == "[]"   run keyword And Continue on skip   SKIP
     # IF   ${source_DB_output1}= "[]"    BREAK
    #  LOG   Sedol has null value
       END
      END

Instrument FundDetails entity attributes validation- Investment sedols
    ${row}=     Read number of rows(sql)  Investment
    ${attribute}=     Read number of rows  FundDetailsInvestment
    ${data}=    Read Sedol from excel sheet  Investment  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Sedol}  Read Sedol from excel sheet  Investment  ${i}  1
        FOR   ${j}    IN RANGE   2     ${attribute}+1
      ${DB Query0}  Read Excel Data of Cell  FundDetailsInvestment  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} '${Sedol}'
      Log to console  ${DB Query}
      ${Source_Table_Script_Output}=              Query      ${DB Query};
      ${GQL_QueryP1}=    Read Excel Data of Cell     FundDetailsInvestment  ${j}  3
      ${GQL_QueryP2}=    Read Excel Data of Cell     FundDetailsInvestment  ${j}  4
      ${GQL_Query}=  Set Variable   ${GQL_QueryP1}${Sedol}"]}}) ${GQL_QueryP2}
      Log to console  ${GQL_Query}
      ${JSON_PATH}=    Read Excel Data of Cell     FundDetailsInvestment  ${j}  5
      #${Source_Table_Script_Output}=              Query      ${DB Query};
      ${source_DB_output}   listToStringWithoutRoundBrackets     ${Source_Table_Script_Output}
      ${source_DB_output1}   listToStringwithoutcotes    ${source_DB_output}
      ${Headers}=     Create dictionary      Content-Type=application/json;charset=utf-8    Accept=application/json;charset=utf-8
