...
Instrument FundManagedByFundManager entity attributes validation- Fund sedols
    ...
    ${attribute}=     Read number of rows  FundManagedByFundManager
    ${data}=    Read Sedol from excel sheet  Funds  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Sedol}  Read Sedol from excel sheet  Funds  ${i}  1
        FOR   ${j}    IN RANGE   2     ${attribute}+1
      ${DB Query0}  Read Excel Data of Cell  FundManagedByFundManager  ${j}  2
      ${DB Query}=  Set Variable   ${DB Query0} ('${Sedol}')
      Log to console  ${DB Query}
      ${Source_Table_Script_Output}=              Query      ${DB Query};
      ${GQL_QueryP1}=    Read Excel Data of Cell     FundManagedByFundManager  ${j}  3
      ${GQL_QueryP2}=    Read Excel Data of Cell     FundManagedByFundManager  ${j}  4
      ${GQL_Query}=  Set Variable   ${GQL_QueryP1}${Sedol}"]}}) ${GQL_QueryP2}
      Log to console  ${GQL_Query}
      ${JSON_PATH}=    Read Excel Data of Cell     FundManagedByFundManager  ${j}  5
      #${Source_Table_Script_Output}=              Query      ${DB Query};
