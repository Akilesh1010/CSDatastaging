*** Settings ***
Documentation       Holding Keywords
...
Resource    ../test_data/Holding.robot
Resource    ../mapping/Holding_getClientPortfolio.robot
...
Holding from Figaro JETS Data Source Portfolio Services end point getClientPortfolio service
    ...
    FOR    ${xml_item}  ${GPL_item}     IN ZIP    ${xml_Holding_getClientPortfolio_attribute}     ${GQL_Holding_getClientPortfolio_attribute}
    ...
    ${GPQL_Response}=      Get Value From Json     ${value}     $.data.account.nodes[0].accountHoldings[0].${GPL_item}
     ${GPQL_Response1}  listToStringwithoutcotes  ${GPQL_Response}
    Should Be Equal As Strings    ${result.status_code}    200
    Log     ${SOAP_Response1}
    Log     ${GPQL_Response1}
        ##Continue For Loop If    ${SOAP_Response} == ${GPQL_Response}
     ${passed}  ${value}  RUN KEYWORD And Ignore error   Should Be Equal As Strings   ${SOAP_Response1}  ${GPQL_Response1}
