...
Instrument FundManager sample entity attributes validation
    ${row}=     Read number of rows  Sample1
    ${data}=    Read Excel Data of Cell   Sample1  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     Sample1    ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     Sample1   ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     Sample1   ${i}  4
...
Instrument FundManagedByFundManager sample entity attributes validation
    ${row}=     Read number of rows  Sample2
    ${data}=    Read Excel Data of Cell   Sample2  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     Sample2    ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell    Sample2    ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell    Sample2    ${i}  4
      ${Source_Table_Script_Output}=              Query      ${DB Query};
      ${source_DB_output}   listToStringWithoutRoundBrackets     ${Source_Table_Script_Output}
      ${source_DB_output1}   listToStringwithoutcotes    ${source_DB_output}
      ${Headers}=     Create dictionary      Content-Type=application/json;charset=utf-8    Accept=application/json;charset=utf-8
