...
Instrument BalanceSheet entity attributes validation
    ${row}=     Read number of rows  BalanceSheet
    ${data}=    Read Excel Data of Cell   BalanceSheet  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     BalanceSheet  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     BalanceSheet  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     BalanceSheet  ${i}  4
