*** Settings ***
Documentation       Person Entity Tax residences Keywords
Library    SoapLibrary
Library    Collections
Library     XML
...
Resource    ../test_data/personEntity_taxresidence.robot
Resource    ../mapping/personentity_taxresidence_getIndividual.robot
...
Person entity taxresidences from Figaro JETS Data Source Client Inception Service Endpoint getIndividual service
    ...
    ${GPQL_Response}=      Get Value From Json     ${value}     $.data.person.nodes[0].taxResidences[0].${GPL_item}
    Should Be Equal As Strings    ${result.status_code}    200
    Log     ${SOAP_Response1}
    Log     ${GPQL_Response}
        ##Continue For Loop If    ${SOAP_Response} == ${GPQL_Response}
    Should Be Equal As Strings     ${SOAP_Response1}   ${GPQL_Response}
    Delete All Sessions
    END

Person Tax residence entity boolean static attrribute validation

    ${row}=     Read number of rows  TaxResidence
    ${data}=    Read Excel Data of Cell   TaxResidence  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Static_Data}=    Read Excel Data of Cell     TaxResidence  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     TaxResidence  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     TaxResidence  ${i}  4
