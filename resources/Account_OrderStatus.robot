*** Settings ***
Documentation       Account Order satus  Keywords
Library    SoapLibrary
Library    Collections
Library     XML
...
Person Entity entitytype figaro OLEDB attributes validation
    ${row}=     Read number of rows  OrderStatus
    ${data}=    Read Excel Data of Cell   OrderStatus  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     OrderStatus  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     OrderStatus  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     OrderStatus  ${i}  4
