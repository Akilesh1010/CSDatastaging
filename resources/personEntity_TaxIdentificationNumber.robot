*** Settings ***
Documentation       Person Entity TaxIdentificationNumber Keywords
Library    SoapLibrary
Library    Collections
Library     XML
...
Person Entity TaxIdentificationNumber figaro OLEDB attributes validation
    ${row}=     Read number of rows  TaxIdentificationNumber
    ${data}=    Read Excel Data of Cell   TaxIdentificationNumber  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     TaxIdentificationNumber  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     TaxIdentificationNumber  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     TaxIdentificationNumber  ${i}  4
