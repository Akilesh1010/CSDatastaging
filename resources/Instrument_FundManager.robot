...
Instrument FundManager sample entity attributes validation
    ${row}=     Read number of rows  FundManager
    ${data}=    Read Excel Data of Cell   FundManager  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     FundManager  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     FundManager  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     FundManager  ${i}  4
