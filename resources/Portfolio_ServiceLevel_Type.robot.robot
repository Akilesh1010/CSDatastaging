*** Settings ***
Documentation       Portfolio Service Level Type  Keywords
Library    SoapLibrary
Library    Collections
Library     XML
...
Portfolio Service Level Type figaro OLEDB attribute validation
    ${row}=     Read number of rows  PortfolioSLType
    ${data}=    Read Excel Data of Cell   PortfolioSLType  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     PortfolioSLType  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     PortfolioSLType  ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     PortfolioSLType  ${i}  4
