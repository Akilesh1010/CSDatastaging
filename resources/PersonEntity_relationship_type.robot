*** Settings ***
Documentation       Person Entity relationship Type Descriptions Entity Keywords
...
Person Entity relationship Type Descriptions entity attributes validation
    ${row}=     Read number of rows  LERTypeStaticData
    ${data}=    Read Excel Data of Cell   LERTypeStaticData  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${Static_Data}=    Read Excel Data of Cell    LERTypeStaticData  ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell      LERTypeTestData  ${i}  2
      ${JSON_PATH}=    Read Excel Data of Cell      LERTypeTestData  ${i}  3
