*** Settings ***
Documentation       Market index constituents Entity Keywords
...
Market index Constituents entity attributes validation
    ${row}=     Read number of rows  MarketIndex_Con
    ${data}=    Read Excel Data of Cell   MarketIndex_Con  1  1
      FOR   ${i}    IN RANGE   2     ${row}+1
      ${DB Query}=    Read Excel Data of Cell     MarketIndex_Con   ${i}  2
      ${GQL_Query}=    Read Excel Data of Cell     MarketIndex_Con   ${i}  3
      ${JSON_PATH}=    Read Excel Data of Cell     MarketIndex_Con   ${i}  4
